...
class ProgrammationSessionSoutenance
{
    ...
    public function getByYear(int $anneeId): array
    {
        $stmt = $this->db->prepare(
            'SELECT id_annee_acad, num_session, date_debut
    ...
    public function upsertSession(int $anneeId, int $numSession, ?string $dateDebut): bool
    {
        $stmt = $this->db->prepare(
            'INSERT INTO programmation_sessions_soutenance (id_annee_acad, num_session, date_debut)
             VALUES (?, ?, ?)
             ON DUPLICATE KEY UPDATE date_debut = VALUES(date_debut)'
        );
        return $stmt->execute([$anneeId, $numSession, $dateDebut]);
    }
