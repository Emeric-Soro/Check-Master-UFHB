...
class Archive
{
    ...
    public function getStudentHistory($anneeAcad = null, $statut = null, $search = null, $limit = 50, $offset = 0)
    {
        ...
        try {
            ...
            $this->logger->error("Error getting student history: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function countStudents($anneeAcad = null, $statut = null, $search = null)
    {
        ...
        try {
            ...
            $this->logger->error("Error counting students: " . $e->getMessage());
            return 0;
        }
    }
    ...
    public function getStudentCompleteFile($numEtu)
    {
        try {
            ...
            $this->logger->error("Error getting complete student file: " . $e->getMessage());
            return null;
        }
    }
    ...
    private function getMoyenneM1($numEtu)
    {
        try {
            ...
            $this->logger->error("Error getting M1 average: " . $e->getMessage());
            return null;
        }
    }
    ...
    private function getMoyenneM2S1($numEtu)
    {
        try {
            ...
            $this->logger->error("Error getting M2 S1 average: " . $e->getMessage());
            return null;
        }
    }
    ...
    private function getStageInfo($numEtu)
    {
        try {
            ...
            $this->logger->error("Error getting stage info: " . $e->getMessage());
            return null;
        }
    }
    ...
    private function getRapportInfo($numEtu)
    {
        try {
            ...
            $this->logger->error("Error getting rapport info: " . $e->getMessage());
            return null;
        }
    }
    ...
    private function getEncadrementInfo($numEtu)
    {
        try {
            ...
            $this->logger->error("Error getting encadrement info: " . $e->getMessage());
            return ['encadrant' => null, 'directeur' => null];
        }
    }
    ...
    private function getSoutenanceInfo($numEtu)
    {
        try {
            ...
            $this->logger->error("Error getting soutenance info: " . $e->getMessage());
            return null;
        }
    }
    ...
    private function getNotesEtudiant($numEtu, $numJury)
    {
        try {
            ...
            $this->logger->error("Error getting student notes: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getJuryHistory($anneeAcad = null, $session = null, $limit = 50, $offset = 0)
    {
        ...
        try {
            ...
            $this->logger->error("Error getting jury history: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getGlobalStats()
    {
        ...
        try {
            $stats['total_students'] = (int)$this->pdo->query("SELECT COUNT(*) FROM etudiants")->fetchColumn();
            $stats['total_soutenances'] = (int)$this->pdo->query("SELECT COUNT(*) FROM programmer WHERE date_soutenance IS NOT NULL AND date_soutenance <> '0000-00-00'")->fetchColumn();
            $stats['total_entreprises'] = (int)$this->pdo->query("SELECT COUNT(DISTINCT id_entreprise) FROM entreprises")->fetchColumn();
            $stats['total_encadreurs'] = (int)$this->pdo->query("SELECT COUNT(DISTINCT id_enseignant) FROM affecter WHERE role = 'encadrant'")->fetchColumn();
        } catch (\PDOException $e) {
            $this->logger->error("Error getting global stats: " . $e->getMessage());
        }
    ...
    public function getYearlyEvolution()
    {
        ...
        try {
            ...
            $this->logger->error("Error getting yearly evolution: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getMentionsDistribution()
    {
        ...
        try {
            ...
            $this->logger->error("Error getting mentions distribution: " . $e->getMessage());
        }
    ...
    public function getTopEntreprises($limit = 10)
    {
        ...
        try {
            ...
            $this->logger->error("Error getting top entreprises: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getAcademicYears()
    {
        ...
        try {
            ...
            $this->logger->error("Error getting academic years: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function updateStudentInfo($numEtu, $data)
    {
        try {
            ...
            $this->pdo->rollBack();
            $this->logger->error("Error updating student info: " . $e->getMessage());
            return false;
        }
    }
    ...
    private function updateRapportInfo($numEtu, $rapportData)
    {
        try {
            // Get the rapport ID
            $stmt = $this->pdo->prepare("SELECT id_rapport FROM rapport_etudiants WHERE num_etu = :num_etu ORDER BY date_rapport DESC LIMIT 1");
            $stmt->execute(['num_etu' => $numEtu]);
            ...
            $this->logger->error("Error updating rapport info: " . $e->getMessage());
        }
    }
