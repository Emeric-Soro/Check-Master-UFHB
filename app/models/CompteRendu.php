...
class CompteRendu
{
    ...
    public function creer($num_etu, $nom_CR, $contenu_CR, $chemin_pdf, $date_CR, $rapports = [])
    {
        try {
            $this->pdo->beginTransaction();
            // Insertion du compte rendu
            $stmt = $this->pdo->prepare("INSERT INTO compte_rendu (num_etu, nom_CR, contenu_CR, chemin_fichier_pdf, date_CR) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$num_etu, $nom_CR, $contenu_CR, $chemin_pdf, $date_CR]);
            $id_CR = $this->pdo->lastInsertId();
            // Liaisons avec les rapports
            if (!empty($rapports)) {
                $stmt2 = $this->pdo->prepare("INSERT INTO compte_rendu_rapport (id_CR, id_rapport) VALUES (?, ?)");
            ...
            $this->pdo->rollBack();
            $this->logger->error("Erreur lors de la création du compte rendu : " . $e->getMessage());
            return false;
        }
    }

    public function getAllArchives($limit = null, $offset = 0, $search = null, $year = null)
    {
        try {
            ...
            $this->logger->error("Erreur lors de la récupération des archives de comptes rendus : " . $e->getMessage());
            return [];
        }
    }

    public function getArchiveById($id_CR)
    {
        try {
            ...
            $this->logger->error("Erreur lors de la récupération de l'archive par ID : " . $e->getMessage());
            return null;
        }
    }

    public function getStatsArchives()
    {
        try {
            $stats = [];
            ...
            $this->logger->error("Erreur lors de la récupération des statistiques d'archives : " . $e->getMessage());
            return [];
        }
    }

    public function deleteArchive($id_CR)
    {
        try {
            $this->pdo->beginTransaction();
            ...
            $this->pdo->rollBack();
            $this->logger->error("Erreur lors de la suppression de l'archive: " . $e->getMessage());
            return false;
        }
    }
}