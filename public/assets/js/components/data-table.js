...
(function (window, document) {
  ...
  var module = (function () {
    ...
    var rowSelector = '.cm-clickable-row, .cm-timeline__item[data-href], table.cm-data-table tbody tr, table.cm-table tbody tr';
    ...
    function findPrimaryAction(row) {
      ...
      addCandidates(candidates, row.querySelectorAll(
      ...
      if (candidates.length === 0) {
        addCandidates(fallbackCandidates, row.querySelectorAll('a[href], button'));
    ...
    function addCandidates(store, nodeList) {
      Array.prototype.forEach.call(nodeList || [], function (candidate) {
    ...
    function getActionScore(action) {
      ...
      if (/(voir|view|consulter|detail|details|fiche|workflow|ouvrir|open|preview|apercu|examiner)/.test(descriptor)) {
        score += 420;
      }
      if (/(edit|modifier|editer|update|traiter|programmer|evaluer)/.test(descriptor)) {
        score += 260;
      }
    ...
    function navigateToHref(href) {
      if (shouldUseAjaxNavigation(href) && window.CM && window.CM.ajax && typeof window.CM.ajax.load === 'function') {
