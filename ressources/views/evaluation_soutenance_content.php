...
foreach ($soutenances as $soutenance) {
    ...
    if (\AcademicYear::isAllSelectedFromSession() && !empty($soutenance['promotion_etu'])) {
        $label .= ' - ' . (string) $soutenance['promotion_etu'];
    }
    ...
    <?php if ($selectedYearLabel !== ''): ?>
        <?php cm_component('ui/alert-box', [
            ...
            'message' => $allYearsSelected
                ? "Affichage multi-années actif. Les nouvelles évaluations restent réservées à l'année académique active {$writeYearLabel}."
                : ($writeAllowed
                    ? "Année académique affichée: {$selectedYearLabel}."
                    : "Consultation historique: {$selectedYearLabel}. Les évaluations sont réservées à l'année académique active {$activeYearLabel}."),
        ]); ?>
    <?php endif; ?>
    ...
    <div class="cm-crud-wrapper">
        <div class="">
            ...
            <form id="cmEvalSoutForm"
                ...
                <div class="cm-grid-2">
                    <?php
                    echo '<input type="hidden" name="id_annee_acad" id="cmEvalAnnee" value="' . htmlspecialchars((string) ($writeYearId ?? ''), ENT_QUOTES, 'UTF-8') . '">';
                ...
                <h4 class="cm-heading" style="margin-top: 1rem; margin-bottom: 0.5rem; font-size: 1rem;">Membres du Jury</h4>
                <div class="cm-grid-5">
                    <?php
                    cm_component('form/input-text', [
                        'name' => 'cm_eval_jury_president',
                        'id' => 'cmEvalJuryPresident',
                        'label' => 'Président',
                        'readonly' => true,
                    ]);
                    cm_component('form/input-text', [
                        'name' => 'cm_eval_jury_examinateur',
                        'id' => 'cmEvalJuryExaminateur',
                        'label' => 'Examinateur',
                        'readonly' => true,
                    ]);
                    cm_component('form/input-text', [
                        'name' => 'cm_eval_jury_directeur',
                        'id' => 'cmEvalJuryDirecteur',
                        'label' => 'Directeur',
                        'readonly' => true,
                    ]);
                    cm_component('form/input-text', [
                        'name' => 'cm_eval_jury_encadreur',
                        'id' => 'cmEvalJuryEncadreur',
                        'label' => 'Encadreur',
                        'readonly' => true,
                    ]);
                    cm_component('form/input-text', [
                        'name' => 'cm_eval_jury_maitre',
                        'id' => 'cmEvalJuryMaitre',
                ...
                <table class="cm-data-table" id="cmEvalSoutTable">
                    ...
                    <tr>
                        <th class="cm-data-table__th cm-data-table__th--check">
                            <input type="checkbox" id="cmEvalSoutCheckAll" aria-label="Tout sélectionner">
                        </th>
                        ...
                        <?php foreach ($rowsToShow as $index => $soutenance): ?>
                            ...
                            <tr class="cm-data-table__row"
                                ...
                                <td class="cm-data-table__td cm-data-table__td--check">
                                    <input type="checkbox" class="cm-eval-sout-check-row" value="<?php echo htmlspecialchars($numEtu, ENT_QUOTES, 'UTF-8'); ?>" aria-label="Sélectionner ligne <?php echo htmlspecialchars($numEtu, ENT_QUOTES, 'UTF-8'); ?>">
                                </td>
                                ...
                                <td class="cm-data-table__td is-center">
                                    <div class="cm-table-actions">
                                        ...
                                        <?php endif; ?>
                                        <?php if ($isEvaluated && (function_exists('canView') ? canView() : true)): ?>
                                            <button type="button"
...
(function () {
    ...
    function fillSoutenanceInfo(numEtu) {
        ...
        if (!info) {
            ...
            const pres = document.getElementById('cmEvalJuryPresident');
            const exam = document.getElementById('cmEvalJuryExaminateur');
            const dir = document.getElementById('cmEvalJuryDirecteur');
            const enc = document.getElementById('cmEvalJuryEncadreur');
            const maitre = document.getElementById('cmEvalJuryMaitre');
            if (pres) pres.value = '';
            if (exam) exam.value = '';
            if (dir) dir.value = '';
            if (enc) enc.value = '';
            if (maitre) maitre.value = '';
        ...
        if (pres) pres.value = info.president_nom || '-';
        if (exam) exam.value = info.examinateur_nom || '-';
        if (dir) dir.value = info.directeur_nom || '-';
        if (enc) enc.value = info.encadreur_nom || '-';
        if (maitre) maitre.value = info.maitre_stage_nom || '-';
        if (selectedLabel) {
    ...
    if (deleteBtn) {
        deleteBtn.addEventListener('click', function () {
            ...
            if (!window.confirm('Supprimer ' + nums.length + ' évaluation(s) ?')) {
                return;
            }
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', function () {
            const headers = ['N', 'Etudiant', 'Promotion', 'Date soutenance', 'Moyenne', 'Mention', 'Commentaire'];
