<?php
$pageSlug = (string) ($_GET['page'] ?? 'parametres_generaux');

cm_render_param_crud_view([
    'page_slug' => $pageSlug,
    'action' => 'fonctions',
    'title' => '',
    'icon' => 'fa-briefcase',
    'form_title_add' => '',
    'form_title_edit' => '',
    'id_key' => 'id_fonction',
    'id_param' => 'id_fonction',
    'list' => is_array($GLOBALS['listeReferentiel'] ?? null) ? $GLOBALS['listeReferentiel'] : [],
    'edit' => $GLOBALS['item_a_modifier'] ?? null,
    'message_success' => (string) ($GLOBALS['messageSuccess'] ?? ''),
    'message_error' => (string) ($GLOBALS['messageErreur'] ?? ''),
    'search_fields' => ['lib_fonction', 'id_fonction'],
    'add_button_name' => 'btn_add_fonctions',
    'edit_button_name' => 'btn_modifier_fonctions',
    'add_button_label' => 'Ajouter',
    'edit_button_label' => 'Modifier',
    'form_fields' => [
        [
            'name' => 'id_fonction',
            'label' => 'Code fonction',
            'type' => 'text',
            'required' => true,
            'placeholder' => 'Ex: AU, CD...',
            'value_key' => 'id_fonction',
        ],
        [
            'name' => 'lib_fonction',
            'label' => 'Libelle fonction',
            'type' => 'text',
            'required' => true,
            'placeholder' => 'Entrez le libelle',
            'value_key' => 'lib_fonction',
        ],
        [
            'name' => 'origine_entreprise',
            'label' => 'Origine entreprise',
            'type' => 'checkbox',
            'required' => false,
            'value_key' => 'origine_entreprise',
        ],
    ],
    'columns' => [
        ['key' => 'id_fonction', 'label' => 'Code'],
        ['key' => 'lib_fonction', 'label' => 'Fonction'],
        ['key' => 'origine_entreprise', 'label' => 'Entreprise ?', 'type' => 'boolean'],
    ],
]);
