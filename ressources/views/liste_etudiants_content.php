    ...
    <header class="cm-flex-between cm-mb-md">
        <div>
            <h2 class="cm-m-0 cm-text-xl cm-text-bold cm-text-primary">
                <i class="fas fa-users cm-mr-sm" aria-hidden="true"></i>
                <?= htmlspecialchars($titre) ?>
            </h2>
            <?php if ($sousTitre): ?>
                <span class="cm-badge cm-badge--success cm-ml-sm"><?= htmlspecialchars($sousTitre) ?></span>
            <?php endif; ?>
            <p class="cm-m-0 cm-text-muted"><?= $totalEtudiants ?> étudiant<?= $totalEtudiants > 1 ? 's' : '' ?> trouvé<?= $totalEtudiants > 1 ? 's' : '' ?></p>
        </div>
    </header>
