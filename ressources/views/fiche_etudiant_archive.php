...
function getMention($note)
{
    if ($note === null)
        return null;
    if ($note >= 16)
        return ['text' => 'Très Bien', 'color' => 'text-purple-600', 'bg' => 'bg-purple-600'];
    if ($note >= 14)
        return ['text' => 'Bien', 'color' => 'text-blue-600', 'bg' => 'bg-blue-600'];
    if ($note >= 12)
        return ['text' => 'Assez Bien', 'color' => 'text-green-600', 'bg' => 'bg-green-600'];
    if ($note >= 10)
        return ['text' => 'Passable', 'color' => 'text-yellow-600', 'bg' => 'bg-yellow-500'];
    ...
    <div class="max-w-5xl mx-auto space-y-8">
        ...
        <?php if ($messageSuccess): ?>
            <div id="success-notice"
                class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded-lg flex justify-between items-center shadow-sm">
                <span><?php echo htmlspecialchars($messageSuccess); ?></span>
                <button onclick="document.getElementById('success-notice').style.display='none'"
            ...
            <div class="flex flex-col sm:flex-row items-start gap-6">
                <div class="flex-grow">
                    <h1 class="text-2xl font-bold text-gray-800">
                        <?php echo htmlspecialchars($studentFile['nom_etu'] . ' ' . $studentFile['prenom_etu']); ?></h1>
                    <p class="text-gray-500">Matricule: <span
                            class="font-medium"><?php echo htmlspecialchars($studentFile['num_etu']); ?></span></p>
                    <p class="text-gray-500">Année académique: <span
                            class="font-medium"><?php echo htmlspecialchars($studentFile['soutenance']['annee_academique'] ?? $studentFile['rapport']['annee_academique'] ?? $studentFile['annee_academique'] ?? 'N/A'); ?></span>
                    </p>
                    <?php if ($mention): ?>
                        <div class="mt-2 text-lg font-bold <?php echo $mention['color']; ?>">
                            ✅ Diplômé - Mention <?php echo $mention['text']; ?>
                            (<?php echo htmlspecialchars(number_format($soutenanceNote, 2)); ?>/20)
                        </div>
                    <?php else: ?>
                        <div class="mt-2 text-lg font-bold text-gray-500">
                            Status: <span
                                class="font-medium"><?php echo htmlspecialchars($studentFile['rapport']['statut_rapport'] ?? 'En cours'); ?></span>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <!-- Timeline -->
        <div class="bg-white rounded-2xl shadow-sm p-6">
            <h3 class="text-lg font-bold text-gray-800 mb-6 flex items-center"><i
                    class="fas fa-stream mr-3 text-blue-500"></i>Parcours de l'étudiant</h3>
                    ...
                    <li class="mb-10 ml-6">
                        <?php if ($step['is_completed']): ?>
                            ...
                            <h4 class="flex items-center mb-1 text-base font-semibold text-gray-900"><?php echo $title; ?>
                                <span
                                    class="text-green-500 bg-green-100 text-sm font-medium mr-2 px-2.5 py-0.5 rounded ml-3">Terminé</span>
                            </h4>
                            <?php
                            ...
                            <?php if ($hasTimelineDate): ?>
                                <time
                                    class="block mb-2 text-sm font-normal leading-none text-gray-400"><?php echo htmlspecialchars(date('d F Y', strtotime($timelineDate))); ?></time>
                            <?php else: ?>
                                <time class="block mb-2 text-sm font-normal leading-none text-gray-400">Date non spécifiée</time>
                            <?php endif; ?>
                        <?php else: ?>
                            <span
                                class="absolute flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full -left-4 ring-8 ring-white">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="space-y-8">
                ...
                <?php if (!empty($studentFile['encadrement']['directeur']) || !empty($studentFile['encadrement']['encadrant'])): ?>
                    <div class="bg-white rounded-2xl shadow-sm p-6">
                        <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center"><i
                                class="fas fa-chalkboard-teacher mr-3 text-blue-500"></i>Encadrement Académique</h3>
                        <dl class="text-sm space-y-2">
                            <?php if (!empty($studentFile['encadrement']['directeur'])): ?>
                                <div class="flex">
                                    <dt class="font-medium text-gray-500 w-32">Directeur</dt>
                                    <dd class="text-gray-900">
                                        <?php echo htmlspecialchars($studentFile['encadrement']['directeur']['nom_enseignant'] . ' ' . $studentFile['encadrement']['directeur']['prenom_enseignant']); ?>
                                    </dd>
                                </div>
                            <?php endif; ?>
                            <?php if (!empty($studentFile['encadrement']['encadrant'])): ?>
                                <div class="flex">
                                    <dt class="font-medium text-gray-500 w-32">Encadrant</dt>
                                    <dd class="text-gray-900">
                                        <?php echo htmlspecialchars($studentFile['encadrement']['encadrant']['nom_enseignant'] . ' ' . $studentFile['encadrement']['encadrant']['prenom_enseignant']); ?>
                                    </dd>
                                </div>
                            <?php endif; ?>
                        </dl>
                ...
                <?php if (!empty($studentFile['stage'])): ?>
                    <div class="bg-white rounded-2xl shadow-sm p-6">
                        <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center"><i
                                class="fas fa-briefcase mr-3 text-blue-500"></i>Informations du Stage</h3>
                        <dl class="text-sm space-y-2">
                            <div class="flex">
                                <dt class="font-medium text-gray-500 w-32">Entreprise</dt>
                                <dd class="text-gray-900">
                                    <?php echo htmlspecialchars($studentFile['stage']['lib_long_entreprise'] ?? 'N/A'); ?>
                                </dd>
                            </div>
                            <div class="flex">
                                <dt class="font-medium text-gray-500 w-32">Maître de stage</dt>
                                <dd class="text-gray-900">
                                    <?php echo htmlspecialchars($studentFile['stage']['encadrant_entreprise'] ?? 'N/A'); ?>
                                </dd>
                            </div>
                            <div class="flex">
                                <dt class="font-medium text-gray-500 w-32">Période</dt>
                                <dd class="text-gray-900">Du
                                    <?php echo htmlspecialchars($studentFile['stage']['date_debut_stage'] ?? 'N/A'); ?> au
                                    <?php echo htmlspecialchars($studentFile['stage']['date_fin_stage'] ?? 'N/A'); ?></dd>
                            </div>
                        </dl>
                    </div>
                <?php endif; ?>
            ...
            <div class="space-y-8">
                ...
                <?php if (!empty($studentFile['soutenance'])): ?>
                    <div class="bg-white rounded-2xl shadow-sm p-6">
                        <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center"><i
                                class="fas fa-graduation-cap mr-3 text-blue-500"></i>Soutenance</h3>
                        <dl class="text-sm space-y-2">
                            <?php
                            ...
                            ?>
                            <div class="flex">
                                <dt class="font-medium text-gray-500 w-32">Date</dt>
                                <dd class="text-gray-900">
                                    <?php echo $hasDateSout ? htmlspecialchars(date('d/m/Y', strtotime($rawDateSout))) : 'N/A'; ?>
                                </dd>
                            </div>
                            <div class="flex">
                                <dt class="font-medium text-gray-500 w-32">Heure</dt>
                                <dd class="text-gray-900">
                                    <?php echo $hasHeureSout ? htmlspecialchars(date('H:i', strtotime($rawHeureSout))) : 'N/A'; ?>
                                </dd>
                            </div>
                            <div class="flex">
                                <dt class="font-medium text-gray-500 w-32">Salle</dt>
                                <dd class="text-gray-900">
                                    <?php echo htmlspecialchars($studentFile['soutenance']['lib_salle'] ?? 'N/A'); ?></dd>
                            </div>
                        </dl>
                    </div>
                <?php endif; ?>
                ...
                <?php if (!empty($studentFile['soutenance']['jury_members'])): ?>
                    <div class="bg-white rounded-2xl shadow-sm p-6">
                        <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center"><i
                                class="fas fa-users mr-3 text-blue-500"></i>Composition du Jury</h3>
                        <dl class="text-sm space-y-2">
                            <?php
                            ...
                            foreach ($juryMembers as $member):
                                ...
                                ?>
                                <div class="flex">
                                    <dt class="font-semibold text-gray-600 w-32 flex-shrink-0">
                                        <?php echo htmlspecialchars($role); ?>:</dt>
                                    <dd class="text-gray-800"><?php echo htmlspecialchars($name); ?></dd>
                                </div>
                            <?php endforeach; ?>
                        </dl>
                    </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6">
            <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center"><i
                    class="fas fa-poll-h mr-3 text-blue-500"></i>Relevé de notes</h3>
            <div class="space-y-4">
                <div class="grid grid-cols-3 items-center gap-4">
                    <span class="text-sm font-medium text-gray-600">Moyenne Master 1 (Annexe 3)</span>
                    <span
                        class="text-sm font-bold text-center"><?php echo $m1_avg ? number_format($m1_avg, 2) : 'N/A'; ?></span>
                    <div class="w-full bg-gray-200 rounded-full h-2.5">
                        <div class="<?php echo getMention($m1_avg)['bg'] ?? 'bg-gray-300' ?> h-2.5 rounded-full"
                ...
                <div class="grid grid-cols-3 items-center gap-4">
                    ...
                    <span
                        class="text-sm font-bold text-center"><?php echo $m2_s1_avg ? number_format($m2_s1_avg, 2) : 'N/A'; ?></span>
                    <div class="w-full bg-gray-200 rounded-full h-2.5">
                        <div class="<?php echo getMention($m2_s1_avg)['bg'] ?? 'bg-gray-300' ?> h-2.5 rounded-full"
                ...
                <div class="grid grid-cols-3 items-center gap-4">
                    ...
                    <span
                        class="text-sm font-bold text-center"><?php echo $memo_avg ? number_format($memo_avg, 2) : 'N/A'; ?></span>
                    <div class="w-full bg-gray-200 rounded-full h-2.5">
                        <div class="<?php echo getMention($memo_avg)['bg'] ?? 'bg-gray-300' ?> h-2.5 rounded-full"
                ...
                <div class="border-t pt-4 grid grid-cols-3 items-center gap-4">
                    ...
                    <span
                        class="text-lg font-bold text-center <?php echo $general_mention['color'] ?? 'text-gray-800' ?>"><?php echo $general_avg ? number_format($general_avg, 2) : 'N/A'; ?></span>
                    <div class="w-full bg-gray-200 rounded-full h-4">
                        <div class="<?php echo $general_mention['bg'] ?? 'bg-gray-300' ?> h-4 rounded-full"
        ...
        <?php if (!empty($studentFile['soutenance']['notes'])): ?>
            <div class="bg-white rounded-2xl shadow-sm p-6">
                <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center"><i
                        class="fas fa-clipboard-list mr-3 text-blue-500"></i>Notes Détaillées de la Soutenance</h3>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <?php foreach ($studentFile['soutenance']['notes'] as $note): ?>
                                <tr>
                                    ...
                                    <td class="px-4 py-2">
                                        <?php echo htmlspecialchars(date('d/m/Y', strtotime($note['date_eval']))); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
        ...
        <?php if (canEdit()): ?>
            <form method="POST" action="?page=admin_historique&action=update_student"
                ...
                <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center"><i
                        class="fas fa-edit mr-3 text-blue-500"></i>Champs Éditables</h3>
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex gap-2">
                                <input type="text" name="nom_etu"
                                    value="<?php echo htmlspecialchars($studentFile['nom_etu']); ?>"
                                    class="flex-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm">
                                <input type="text" name="prenom_etu"
                                    value="<?php echo htmlspecialchars($studentFile['prenom_etu']); ?>"
                                    class="flex-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm">
                            </div>
                        </div>
                        <div>
                            <label for="email_etu" class="block text-sm font-medium text-gray-700">Email</label>
                            <input type="email" name="email_etu"
                                value="<?php echo htmlspecialchars($studentFile['email_etu']); ?>"
                    ...
                    <?php if (!empty($studentFile['rapport'])): ?>
                        <div class="space-y-4 border-t border-gray-200 pt-6">
                            ...
                            <div>
                                <label for="theme_rapport" class="block text-sm font-medium text-gray-700">Thème du
                                    mémoire</label>
                            ...
                            <div>
                                <label for="statut_rapport" class="block text-sm font-medium text-gray-700">Statut de
                                    validation</label>
                                <select name="statut_rapport" id="statut_rapport"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm">
                ...
                <div class="flex justify-end gap-4 mt-8 pt-4 border-t border-gray-200">
                    <a href="?page=admin_historique"
                        class="bg-white hover:bg-gray-100 text-gray-700 font-medium py-2 px-4 border border-gray-300 rounded-lg shadow-sm">Annuler</a>
                    <button type="submit"
